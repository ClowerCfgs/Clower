...
def handle_rechazar(message):
    ...
    if message.from_user.id == admin_id:
        try:
            user_id = int(message.text.split()[1])
            tipo = message.text.split()[2]
            if tipo == "inversión" and user_id en pendientes:
            ...
            elif tipo == "retiro" y user_id en retiros_pendientes:
...
# Configuración para escuchar en el puerto esperado por Render
